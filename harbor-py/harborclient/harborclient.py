...
class HarborClient(object):
    def __init__(self, host,port, user, password, protocol="http"):
        ...
        login_data = requests.post("%s://%s:%s/login" % (self.protocol, self.host, self.port),
                     data={'principal': self.user,'password': self.password },verify=False)
        if login_data.status_code == 200:
            self.session_id = login_data.cookies['beegosessionID']
    ...
    def logout(self):
        requests.get('%s://%s:%s/log_out' % (self.protocol, self.host, self.port),
    ...
    def search(self, query_string):
        result = None
        path = '%s://%s:%s/api/search?q=%s' % (self.protocol, self.host,self.port, query_string)
    ...
    def get_projects(self):
        result = None
        path = '%s://%s:%s/api/projects' % (self.protocol, self.host, self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_project_members(self, project_id):
        result = None
        path = '%s://%s:%s/api/projects/%s/members' % (self.protocol, self.host,self.port, project_id)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def delete_member_from_project(self, project_id, user_id):
        result = False
        path = '%s://%s:%s/api/projects/%s/members/%s' % (self.protocol, self.host, self.port,
                                                       project_id, user_id)
    ...
    def set_project_publicity(self, project_id, is_public):
        result = False
        path = '%s://%s:%s/api/projects/%s/publicity' % (
            self.protocol, self.host,self.port, project_id)
    ...
    def get_statistics(self):
        result = None
        path = '%s://%s:%s/api/statistics' % (self.protocol, self.host, self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_users(self):
        ...
        result = None
        path = '%s://%s:%s/api/users' % (self.protocol, self.host, self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_user_info(self):
        result = None
        path = '%s://%s:%s/api/users/current' % (self.protocol, self.host,self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def update_user_profile(self, user_id, email, realname, comment):
        ...
        result = False
        path = '%s://%s:%s/api/users/%s' % (self.protocol, self.host,self.port.
                                                    user_id)
    ...
    def change_password(self, user_id, old_password, new_password):
        result = False
        path = '%s://%s:%s/api/users/%s/password' % (
            self.protocol, self.host,self.port, user_id)
    ...
    def promote_as_admin(self, user_id, has_admin_role):
        result = False
        path = '%s://%s:%s/api/users/%s/sysadmin' % (
            self.protocol, self.host,self.port, user_id)
    ...
    def get_repositories(self, project_id, query_string=None):
        # TODO: support parameter
        result = None
        path = '%s://%s:%s/api/repositories?project_id=%s' % (
            self.protocol, self.host,self.port, project_id)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
    ...
    def delete_tag_of_repository(self, repo_name, tag):
        result = False
        path = '%s://%s:%s/api/repositories/%s/tags/%s' % (self.protocol,self.host,self.port,
                                                        repo_name, tag)
        response = requests.delete(path,
                                   cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
    ...
    def delete_tags_of_repository(self, repo_name):
        result = False
        path = '%s://%s:%s/api/repositories/%s/tags' % (self.protocol,
                                                     self.host,self.port, repo_name)
        response = requests.delete(path,
                                   cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = True
    ...
    def get_repository_tags(self, repo_name):
        result = None
        path = '%s://%s:%s/api/repositories/%s/tags' % (
            self.protocol, self.host,self.port, repo_name)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_repository_manifest(self, repo_name, tag):
        result = None
        path = '%s://%s:%s/api/repositories/%s/tags/%s/manifest' % (
            self.protocol, self.host,self.port, repo_name, tag)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_top_accessed_repositories(self, count=None):
        result = None
        path = '%s://%s:%s/api/repositories/top' % (self.protocol, self.host,self.port)
        if count:
            path += "?count=%s" % (count)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
    ...
    def get_logs(self, lines=None, start_time=None, end_time=None):
        result = None
        path = '%s://%s:%s/api/logs' % (self.protocol, self.host,self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_systeminfo(self):
        result = None
        path = '%s://%s:%s/api/systeminfo' % (self.protocol, self.host,self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
    ...
    def get_configurations(self):
        result = None
        path = '%s://%s:%s/api/configurations' % (self.protocol, self.host,self.port)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id}, verify=False)
        if response.status_code == 200:
            result = response.json()
